...
def test_full_polars_cleanup():
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as z:
            ...
            print(f"  [2] Setting up Polars lazy frame...")
            lazy_df = pl.scan_csv(
                extracted_txt_path,
                separator='|',
                has_header=False,
            ...
            print(f"  [3] Sinking to Parquet...")
            lazy_df.sink_parquet(parquet_path)
            
            print(f"  [4] Cleaning up Polars references...")
            del lazy_df
            pl.StringCache().clear()
            gc.collect()
            
            print(f"  [5] Attempting to delete extracted txt within zip context...")
            try:
