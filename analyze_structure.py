...
def analyze_with_polars():
    base_dir = r'e:/국건위-auri/2026 국건위 업무(안의순)/10. 기획단 업무/도심형 블록주택(도심주택)/0223_세움터 데이터 분석'
    schema_file = os.path.join(base_dir, 'data/schema/schema_총괄표제부.txt')
    zip_path = os.path.join(base_dir, 'data/original/국토교통부_건축물대장_총괄표제부+(2025년+12월).zip')
    
    print(f"Loading schema from {schema_file}")
    columns, read_dtypes, cast_dtypes = load_schema(schema_file)
    print(f"Mapped {len(columns)} columns. Scheduled {len(cast_dtypes)} for Decimal casting.")
